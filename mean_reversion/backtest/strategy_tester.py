...
def _calc_quant_projections(up_moves, dn_moves, vols, is_valid, lookback, min_samples):
    n = len(up_moves)
    # Output: up50, up75, up90, up95, dn50, dn75, dn90, dn95
    proj = np.full((n, 8), np.nan)
    ...
    for i in range(n):
        ...
        if count >= min_samples and cur_vol > 0:
            ...
            proj[i, 3] = _pine_percentile(scaled_up, 95)
            proj[i, 4] = _pine_percentile(scaled_dn, 50)
            proj[i, 5] = _pine_percentile(scaled_dn, 75)
            proj[i, 6] = _pine_percentile(scaled_dn, 90)
            proj[i, 7] = _pine_percentile(scaled_dn, 95)
